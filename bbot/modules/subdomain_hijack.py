...
class subdomain_hijack(BaseModule):
    ...
    def setup(self):
        ...
        for f in fingerprints:
            try:
                ...
                continue
            if not (f.cnames and f.vulnerable and f.fingerprint and f.autocheck_pass):
    ...
    def check_subdomain(self, subdomain):
        for f in self.fingerprints:
            for domain in f.cnames:
                if self.helpers.host_in_host(subdomain, domain):
                    url = f"https://{subdomain}"
                    try:
                        r = self.helpers.request(url)
                        if not f.nxdomain and f.fingerprint_regex.findall(r.text):
...
class Fingerprint:
    def __init__(self, fingerprint):
        ...
        self.vulnerable = fingerprint.get("status", "Not vulnerable").capitalize() == "Vulnerable"
