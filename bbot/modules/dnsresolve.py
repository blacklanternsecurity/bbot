...
class dnsresolve(BaseModule):
    ...
    watched_events = ["IPV4_ADDRESS", "IPV4_RANGE", "IPV6_ADDRESS", "IPV6_RANGE", "DNS_NAME"]
        ...
        if event.type == "DNS_NAME":
            ...
            if (event.type == "IPV4_RANGE") or (event.type == "IPV6_RANGE"):
                ...
                if net.num_addresses > config_max_hosts:
                    self.debug(f"dns resolve exceeded max host count ({config_max_hosts} hosts). Got a (/{net.prefixlen}) network with ({net.num_addresses:,}) hosts.")
                    return
    ...
    def resolve_arr(self,arr,event):
        futures = []
        for x in arr:
            future = self.helpers.run_async(self.helpers.resolve,str(x))
