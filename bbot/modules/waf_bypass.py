...
class waf_bypass(BaseModule):
    ...
    async def handle_event(self, event):
        ...
        if is_protected:
            ...
            simhash = await self.helpers.run_in_executor_mp(compute_simhash, curl_response["response_data"])
    ...
    async def check_ip(self, ip, source_domain, protected_domain, source_event):
        ...
        bypass_simhash = await self.helpers.run_in_executor_mp(compute_simhash, bypass_response["response_data"])
