...
def find_common_prefixes(strings, minimum_set_length=4):
    ...
    for prefix in prefix_list:
        ...
        for k, v in frequency_dict.items():
            if prefix != k and prefix in k:
                is_substring = True
        if not is_substring:
            found_prefixes.add(prefix)
        elif prefix_frequency > v and (len(k) - len(prefix) == 1):
            found_prefixes.add(prefix)
...
class ffuf_shortnames(ffuf):
    ...
    async def setup(self):
        ...
        wordlist = self.config.get("wordlist", "") or f"{self.helpers.wordlist_dir}/ffuf_shortname_candidates.txt"
        ...
        wordlist_extensions = self.config.get("wordlist_extensions", "") or f"{self.helpers.wordlist_dir}/raft-small-extensions-lowercase_CLEANED.txt"
    ...
    async def handle_event(self, event):
        ...
        if self.config.get("find_delimeters"):
            if "shortname-directory" in event.tags:
                if delimeter_r := self.find_delimeter(filename_hint):
    ...
    async def finish(self):
        if not self.config.get("find_common_prefixes"):
            return
