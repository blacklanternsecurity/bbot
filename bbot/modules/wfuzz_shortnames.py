...
class wfuzz_shortnames(wfuzz):
    watched_events = ["URL_HINT"]
    produced_events = ["URL_UNVERIFIED"]
    flags = ["aggressive", "active", "iis-shortnames", "web-thorough"]
    meta = {"description": "Use wfuzz in combination IIS shortnames"}
    ...
    options_desc = {
        ...
        "find_common_prefixes": "Attempt to automatically detect common prefixes and make additional wfuzz runs against them",
        "find_delimeters": "Attempt to detect common delimeters and make additional wfuzz runs against them",
    }
    ...
    def setup(self):
        ...
        if not wordlist:
            wordlist = f"{self.helpers.wordlist_dir}/wfuzz_shortname_candidates.txt"
        ...
        if event.source.type == "URL":
            ...
            if tempfile_len > 0:
                if "shortname-file" in event.tags:
                    ...
                    for r in self.execute_wfuzz(tempfile, root_url, exts=["/"]):
                        r_url = f"{r['url'].rstrip('/')}/"
                        self.emit_event(r_url, "URL_UNVERIFIED", source=event, tags=[f"status-{r['code']}"])
            ...
            if self.config.get("find_delimeters"):
                if "shortname-directory" in event.tags:
                    ...
                    if delimeter_r:
                        ...
                        for r in self.execute_wfuzz(tempfile, root_url, prefix=f"{prefix}{delimeter}", exts=["/"]):
                            self.emit_event(r["url"], "URL_UNVERIFIED", source=event, tags=[f"status-{r['code']}"])
                        ...
                        if hint.startswith(prefix):
                            ...
                            if len(partial_hint) > 0:
                                ...
                                if "shortname-directory" in self.shortname_to_event[hint].tags:
                                    ...
                                    for r in self.execute_wfuzz(tempfile, url, prefix=prefix, exts=["/"]):
                                        self.emit_event(
                                            r["url"],
                                            "URL_UNVERIFIED",
                                            source=self.shortname_to_event[hint],
                                            tags=[f"status-{r['code']}"],
                                        )
                                    ...
                                    for ext in used_extensions:
                                        ...
                                        for r in self.execute_wfuzz(tempfile, url, prefix=prefix, suffix=f".{ext}"):
                                            self.emit_event(
                                                r["url"],
                                                "URL_UNVERIFIED",
                                                source=self.shortname_to_event[hint],
