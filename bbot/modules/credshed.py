...
class credshed(BaseModule):
    ...
    flags = ["passive"]
    meta = {"description":"Send queries to your own credshed server to check for known credentials of your targets", "auth_required": True}
    options = {"username":"", "password":"", "credshed_url":""}


    async def setup(self):
        self.base_url = self.config.get("credshed_url","")
        self.username = self.config.get("username","")
        self.password = self.config.get("password","")
        self.results = {}

        auth_setup = await self.helpers.request(f"{self.base_url}/api/auth",
                                      method="POST",
                                      json={
    ...
    async def handle_event(self, event):
        cs_query = await self.helpers.request(f"{self.base_url}/api/search", method="POST", cookies={'access_token_cookie':self.auth_token}, json={'query':event.data})
                ...
                if cs_query.json().get('stats').get('query_type') == "domain":
                   ...
                   if self.results[x]["hashed"]:
                       for y in self.results[x]["hashed"]:
                           self.emit_event(y, "HASHED_PASSWORD",source=f, tags=f'credshed-source-{self.results[x]["hashed"][y]}')

                   if self.results[x]["passwords"]:
                       for y in self.results[x]["passwords"]:
                           self.emit_event(y, "PASSWORD", source=f, tags=f'credshed-source-{self.results[x]["passwords"][y]}')
                    
                   if self.results[x]["usernames"]:
                       for y in self.results[x]["usernames"]:
                           self.emit_event(y, "USERNAME", source=f, tags=f'credshed-source-{self.results[x]["usernames"][y]}')

                if cs_query.json().get('stats').get('query_type') == "email":
                    if self.results[x]["hashed"]:
                       for y in self.results[x]["hashed"]:
                           self.emit_event(y, "HASHED_PASSWORD",source=event, tags=f'credshed-source-{self.results[x]["hashed"][y]}')

                    if self.results[x]["passwords"]:
                       for y in self.results[x]["passwords"]:
                           self.emit_event(y, "PASSWORD", source=event, tags=f'credshed-source-{self.results[x]["passwords"][y]}')
                    
                    if self.results[x]["usernames"]:
                       for y in self.results[x]["usernames"]:
                           self.emit_event(y, "USERNAME", source=event, tags=f'credshed-source-{self.results[x]["usernames"][y]}')