...
class dehashed(BaseModule):
    ...
    options_desc = {"username":"Email Address associated with your API key","api_key": "API Key", "check_domains":"Enable only if bbot should search dehashed for new email addresses - this can be significantly more expensive against the API."}
    ...
    async def handle_event(self, event):
        ...
        if self.api_warn:
            self.info(f"{event.data} has {self.api_count} results in Dehashed. The API can only process the first 30,000 results. Please check dehashed.com to get the remaining results.")
