...
class BBOTArgumentParser(argparse.ArgumentParser):
    ...
    def __init__(self, *args, **kwargs):
        if not self._dummy:
            module_loader = kwargs.pop("_module_loader")
            self._module_choices = sorted(set(module_loader.configs(type="scan")))
            self._output_module_choices = sorted(set(module_loader.configs(type="output")))
