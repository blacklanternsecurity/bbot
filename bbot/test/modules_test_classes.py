...
class Telerik(HttpxMockHelper):
    ...
    def mock_args(self):
        ...
        respond_args = {"response_data": '{ "message" : "RadAsyncUpload handler is registered succesfully, however, it may not be accessed directly." }'}
        ...
        respond_args = {"response_data": "<input type=\"hidden\" name=\"dialogParametersHolder\" id=\"dialogParametersHolder\" /><div style='color:red'>Cannot deserialize dialog parameters. Please refresh the editor page.</div><div>Error Message:Invalid length for a Base-64 char array or string.</div></form></body></html>"}
        self.set_expect_requests(expect_args=expect_args, respond_args=respond_args)
    ...
    def check_events(self, events):
        ...
        if telerik_axd_detection and telerik_axd_vulnerable and telerik_spellcheck_detection and telerik_dialoghandler_detection:
            return True
        return False
