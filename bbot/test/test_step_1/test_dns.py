...
def test_dnsbrute_canary_check():
    ...
    # for membership checks after being converted to a list.

    def fake_gen(n=5):
        for i in range(n):
            yield f"canary{i}"

    # Bug 1: exhausted generator -- `sub in canaries` always returns False
    canaries_gen = fake_gen(5)
    canaries_list = list(canaries_gen)
    # After list(), the generator is exhausted; `in` on it always returns False
    assert "canary0" not in canaries_gen, "generator should be exhausted"

    # The fix: use a set built from the list for O(1) membership checks
    canary_set = set(canaries_list)
    assert "canary0" in canary_set
    assert "canary4" in canary_set
    assert "legit" not in canary_set

    # Bug 2: trailing dot from hostname.split(domain)
    domain = "example.com"
    hostname = "canary0.example.com"
    sub_buggy = hostname.split(domain)[0]
    assert sub_buggy == "canary0.", "split produces trailing dot"
    assert sub_buggy not in canary_set, "trailing dot prevents match"

    # The fix: strip the trailing dot
    sub_fixed = sub_buggy.rstrip(".")
    assert sub_fixed == "canary0"
    assert sub_fixed in canary_set, "after stripping dot, canary is found"

    # Bug 3: variable shadowing in wildcard_domains loop
    original_domain = "example.com"
    wildcard_domains = {
        "example.com": {"A", "CNAME"},
        "sub.example.com": {"A"},
    }
    # Simulating the buggy code: `for domain, rdtypes in wildcard_domains.items()`
    # would overwrite the `domain` variable
    domain = original_domain
    for domain, rdtypes in wildcard_domains.items():
        pass
    assert domain != original_domain or len(wildcard_domains) <= 1, "domain variable gets shadowed"

    # The fix: use a different loop variable
    domain = original_domain
    for _wd, rdtypes in wildcard_domains.items():
        pass
    assert domain == original_domain, "original domain is preserved with renamed loop var"

    # End-to-end simulation: canary detection with the fix applied
    canaries_list = [f"canary{i}" for i in range(10)]
    canary_set = set(canaries_list)
    wordlist = ["www", "mail", "api"]
    subdomains = canaries_list[:5] + wordlist + canaries_list[5:]

    # Simulate massdns returning ALL subdomains (wildcard domain)
    massdns_results = [f"{sub}.example.com" for sub in subdomains]

    results = []
    canaries_triggered = []
    for hostname in massdns_results:
        sub = hostname.split("example.com")[0].rstrip(".")
        if sub in canary_set:
            canaries_triggered.append(sub)
        else:
            results.append(hostname)

    assert len(canaries_triggered) == 10, f"all 10 canaries should be caught, got {len(canaries_triggered)}"
    assert len(results) == 3, f"only legitimate subdomains should remain, got {len(results)}"
    assert all("canary" not in r for r in results), "no canary subdomains should leak into results"
