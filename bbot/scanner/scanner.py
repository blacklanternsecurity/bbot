...
from bbot.core.helpers import kill_children
...
class Scanner:
    ...
    def __init__(self, scan_id, *targets, modules=None, config=None):
        ...
        self.shared_thread_pool = ThreadPool(threads=config.get('max_threads', 100), name='scanner_shared_thread_pool')
        ...
        self.modules = OrderedDict(sorted(self.modules.items(), key=lambda x: getattr(x[-1], 'priority', 0)))
        if self.modules:
        ...
        try:
            ...
            for module_name, module in self.modules.items():
                try:
                    ...
                    import traceback
                    self.error(f'Failed to setup module {module_name}:\n{traceback.format_exc()}')
            if not self.modules:
