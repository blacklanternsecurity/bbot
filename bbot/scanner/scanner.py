...
class Scanner:
    ...
    async def async_start(self):
        ...
        try:
            ...
            # start status ticker
            self.ticker_task = asyncio.create_task(self._status_ticker(self.status_frequency), name=f"{self.name}._status_ticker()")
            ...
            self.init_events_task = asyncio.create_task(self.ingress_module.init_events(self.target.events), name=f"{self.name}.ingress_module.init_events()")
            ...
            if self.status == "ABORTING" and not status == "ABORTED":
                ...
                if status != self._status:
                    ...
                    self.dispatcher_tasks.append(
                        asyncio.create_task(self.dispatcher.catch(self.dispatcher.on_status, self._status, self.id), name=f"{self.name}.dispatcher.on_status({status})")
                    )
                else:
