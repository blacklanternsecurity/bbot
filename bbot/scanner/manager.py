...
class EventManager:
    ...
    def init_events(self):
        ...
        '''
        for event in self.scanner.target.events:
            self.scanner.info(f'Target: {event}')
            self.queue_event(event)
        # force submit batches
        for mod in self.scanner.modules.values():
    ...
    def queue_event(self, event):
        ...
        if hash(event) in self.event_hashes:
            self.scanner.verbose(f'Duplicate event: {event}')
            return
    ...
    def modules_status(self, _log=False):
        ...
        while passes > 0:
            ...
            for m in self.scanner.modules.values():
                try:
                    if m.event_queue:
            ...
            queued_tasks = sorted(queued_tasks.items(), key=lambda x: x[-1], reverse=True)
        ...
        if _log:
            ...
            if not events_queued:
                events_queued = 'None'
            tasks_queued = ', '.join([f'{mod}: {qsize:,}' for mod, qsize in queued_tasks[:5] if qsize > 0])
            if not tasks_queued:
                tasks_queued = 'None'
            self.scanner.debug(f'Events queued: {sum([m[-1] for m in queued_events]):,} ({events_queued})')
            self.scanner.debug(f'Tasks queued: {sum([m[-1] for m in queued_tasks]):,} ({tasks_queued})')
            if modules_running:
                self.scanner.debug(f'Modules running: {len(modules_running):,} ({", ".join(modules_running)})')
            if modules_errored:
                self.scanner.debug(f'Modules errored: {len(modules_errored):,} ({", ".join(modules_errored)})')
